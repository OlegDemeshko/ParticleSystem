...
def simulation(steps=500):
    ps = ParticleSystem()
    T_list, P_list, P_virial_list = [], [], []
    temp_sum, pressure_sum = 0, 0

    for step in range(steps):
        ke, pe = ps.verlet_step()
        temp = ke / ps.N / 1.5
        temp_sum += temp
        pressure_sum += ke / 1.5 / (ps.Lx * ps.Ly)

        if step % 5 == 0:
            T_list.append(temp_sum / 5)
            P_list.append(pressure_sum / 5)
            P_virial_list.append(ps.compute_pressure(temp_sum / 5))
            temp_sum = 0
            pressure_sum = 0

    plt.plot(T_list, P_list, label="Pressure")
    plt.plot(T_list, P_virial_list, label="Virial Pressure")
